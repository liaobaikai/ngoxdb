...
public abstract class BasicDatabaseComparator implements DatabaseComparator {
    ...
    public void compareAllTables(List<TableInfo> tis, OnComparingListener onComparingListener) {
        for (TableInfo ti : tis) {
            ...
            final String table = this.getDatabaseConverter().getRightName(ti.getTableName());

            // 迁移表的数据
            // 查询原系统的数据
            int pageSize = this.getDatabaseConverter().getDatabaseConfig().getPageSize();
            // 获取表的行数
            long tableRowCount = this.getDatabaseConverter().getDatabaseDao().getTableRowCount(ti.getTableSchem(), table);
            if (tableRowCount == 0) {
                // 无数据
            ...
            for (int i = 0, offset, limit; i < tablePageSize; i++) {
                ...
                List<Object[]> batchArgs = this.getDatabaseConverter().queryData(this.getDatabaseConverter().getPaginationSQL(ti, offset, limit));
    ...
    public ComparisonResult compare(TableInfo ti,
        ...
        for (int i = 0; i < sourceCount; i++) {
            ...
            Object[] tr;
            if (uniqueKeyIndex >= 0) {
                if (sr[uniqueKeyIndex].equals("FA594384-1E98-497E-92FE-BE9003C88F9D")) {
                    System.out.println();
                }
